...
export default function App() {
  ...
  const interviewCompletedRef = useRef(false); // ✅ naya ref
  ...
  useEffect(() => {
    const handleFullscreenChange = async () => {
      // ✅ Interview complete ho gaya toh guard bilkul trigger mat karo
      if (interviewCompletedRef.current) return;

      if (currentStep >= 9 && currentStep < 10 && !document.fullscreenElement && !fullscreenViolated) {
        setFullscreenViolated(true);
        toast.error("Fullscreen exited. Interview has been terminated.", { duration: 6000 });
        try {
          stopScreenShare();
  ...
  useEffect(() => {
    const blockEsc = (e: KeyboardEvent) => {
      if (e.key === "Escape" && currentStep >= 7 && currentStep <= 9) {
        e.preventDefault();
        e.stopPropagation();
      }
    };
  ...
  const expireLinkAPI = async (email: string) => {
    try {
      ...
      if (!response.ok) throw new Error('Link expiry API call failed');
  ...
  const handleInterviewComplete = async () => {
    try {
      ...
      toast.success("Finalizing interview…");

      // ✅ Pehle mark karo — taaki fullscreen guard trigger na ho
      ...
      // ✅ Error pe reset karo taaki dobara try ho sake
  ...
  const handleRestart = () => {
    ...
    setAnswers([]);
    setCompletedTime("");
    interviewCompletedRef.current = false; // ✅ reset
  };
            ...
            <motion.div key="step7-guidelines" {...stepMotionProps}>
              <StepGuidelines
                onContinue={async () => {
                  ...
                  if (screenStreamRef.current) startScreenRecording(screenStreamRef.current);
                  setCurrentStep(8);
                }}
              />
            </motion.div>
...
//     document.addEventListener("fullscreenchange", handleFullscreenChange);
//     return () => document.removeEventListener("fullscreenchange", handleFullscreenChange);
//   }, [currentStep, fullscreenViolated, screenStream]);
//   // useEffect(() => {
//   //   const handleFullscreenChange = async () => {
//   //     if (currentStep >= 9 && !document.fullscreenElement && !fullscreenViolated) {
//   //       setFullscreenViolated(true);
...
//       // ✅ Turant screen share band karo — browser ka indicator hatega
//       if (screenStream) {
//         screenStream.getTracks().forEach((track) => track.stop());
//         setScreenStream(null);
//       }
