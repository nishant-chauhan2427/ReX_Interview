...
import { User, AlertCircle } from "lucide-react"; // ✅ Change 1: AlertCircle icon add kiya
...
export function Step2UserDetails({
  ...
  const [error, setError] = useState<string | null>(null); // ✅ Change 2: error state add kiya
  ...
  const handleSubmit = async (e: FormEvent<HTMLFormElement>) => {
    ...
    setIsLoading(true);
    setError(null); // ✅ Har submit pe purana error clear karo
    ...
    try {
      ...
      setError(err?.message || "Something went wrong. Please try again."); // ✅ Change 3: error dikhao
    } finally {
      setIsLoading(false);
    }
  };
      ...
      >
        <div className="glass-card rounded-3xl p-10">
          ...
          <form onSubmit={handleSubmit} className="space-y-5">
            ...
            {/* ✅ Error box — sirf tab dikhega jab error ho */}
            {error && (
              <motion.div
                initial={{ opacity: 0, y: -8 }}
                animate={{ opacity: 1, y: 0 }}
                className="flex items-start gap-3 px-4 py-3 rounded-lg bg-red-500/10 border border-red-500/30"
              >
            ...
            <motion.button
              type="submit"
              disabled={isLoading}
              className="w-full px-6 py-4 bg-primary text-primary-foreground rounded-lg disabled:opacity-60 disabled:cursor-not-allowed"
